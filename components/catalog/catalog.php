<!-- table -->
<!-- <?php
if (isset($catalog_mode) && $catalog_mode == "Terbaru") {
    echo "Buku Terbaru";
} else {
    echo "Normal";
}
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        <div class="col-md-3 mb-4">
            <div class="card h-100">
                ...
                ?>
                <img src="<?= str_replace($root, '/Perpustakaan', $gambar_path) ?>" class="card-img-top"
                    alt="<?= htmlspecialchars($row['nama_buku'] ?? 'Tidak ada judul') ?>"
                    style="height: 250px; object-fit: cover;">
                <div class="card-body d-flex flex-column">
                    <h5 class="card-title text-truncate">
                        <?= htmlspecialchars($row['nama_buku'] ?? 'Tidak ada judul') ?>
                    </h5>
                    <p class="card-text">
