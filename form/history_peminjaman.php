	...
	<div class="row">
		...
		<table class="table table-striped table-bordered table-hover">
		  ...
		  <?php
			...
			$sql_det = "SELECT p.*, 
						(SELECT COUNT(*) FROM t_detil_pinjam WHERE id_t_peminjaman = p.id_t_peminjaman) as jum,
						CASE 
							WHEN p.id_t_staff IS NOT NULL THEN CONCAT('Staff-', s.nama)
							ELSE 'Admin'
						END as staff
						FROM t_peminjaman p
						LEFT JOIN t_staff s ON p.id_t_staff = s.id_t_staff
						WHERE p.id_t_anggota = $id_ag 
